...
def web_menu():
    while True:
        ...
        if response == '1':
            target_url = input('veuillez saisir le target: ')
            https_url_check = gobuster.ensure_https(target_url)
            try:
                gobuster_results = gobuster.scan_directories(https_url_check)
            ...
            target_url = input("Veuillez taper un url: ")
            https_url_check = nikto.ensure_https(target_url)
            try:
                nikto.run_nikto_scan(https_url_check)
            finally:
